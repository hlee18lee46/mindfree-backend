...
import { assertDefined, assertIsContractAddress, fromHex, parseCoinPublicKeyToHex, assertUndefined, toHex } from '@midnight-ntwrk/midnight-js-utils';
import { getRuntimeNetworkId, getLedgerNetworkId, getZswapNetworkId } from '@midnight-ntwrk/midnight-js-network-id';
 ...
 */
function createUnprovenCallTxFromInitialStates(options) {
...
async function createUnprovenCallTx(providers, options) {
    ...
    if (hasPrivateStateId && hasPrivateStateProvider) {
        ...
        return createUnprovenCallTxFromInitialStates(createCallOptions(options, parseCoinPublicKeyToHex(providers.walletProvider.coinPublicKey, getZswapNetworkId()), contractState, zswapChainState, privateState));
    }
    const { zswapChainState, contractState } = await getPublicStates(providers.publicDataProvider, options.contractAddress);
    return createUnprovenCallTxFromInitialStates(createCallOptions(options, parseCoinPublicKeyToHex(providers.walletProvider.coinPublicKey, getZswapNetworkId()), contractState, zswapChainState));
}
 ...
 */
function createUnprovenDeployTxFromVerifierKeys(verifierKeys, coinPublicKey, options) {
    ...
    const [contractAddress, initialContractState, unprovenTx] = createUnprovenLedgerDeployTx(verifierKeys, options.signingKey, nextContractState, nextZswapLocalState);
    return {
        public: {
            contractAddress,
            initialContractState
...
async function createUnprovenDeployTx(providers, options) {
    ...
    return createUnprovenDeployTxFromVerifierKeys(verifierKeys, parseCoinPublicKeyToHex(providers.walletProvider.coinPublicKey, getZswapNetworkId()), options);
}
