...
async function createUnprovenCallTx(providers, options) {
    ...
    if (hasPrivateStateId && hasPrivateStateProvider) {
        ...
        return createUnprovenCallTxFromInitialStates(createCallOptions(options, midnightJsUtils.parseCoinPublicKeyToHex(providers.walletProvider.coinPublicKey, midnightJsNetworkId.getZswapNetworkId()), contractState, zswapChainState, privateState));
    }
    const { zswapChainState, contractState } = await getPublicStates(providers.publicDataProvider, options.contractAddress);
    return createUnprovenCallTxFromInitialStates(createCallOptions(options, midnightJsUtils.parseCoinPublicKeyToHex(providers.walletProvider.coinPublicKey, midnightJsNetworkId.getZswapNetworkId()), contractState, zswapChainState));
}
...
async function createUnprovenDeployTx(providers, options) {
    ...
    return createUnprovenDeployTxFromVerifierKeys(verifierKeys, midnightJsUtils.parseCoinPublicKeyToHex(providers.walletProvider.coinPublicKey, midnightJsNetworkId.getZswapNetworkId()), options);
}
